TF.Screens.dashboard = function(root) {
  ...
  if (meta) {
    meta.innerHTML = '<span style="font-family:var(--font-m);font-size:11px">' +
  ...
  root.innerHTML = '<div class="screen">' +
    ...
    '<div class="section">' +
      ...
      '<div id="quote-card" class="quote-card"><div class="t-hint" style="font-style:italic;animation:pulse 1.4s infinite">Fetching a live Stoic quote...</div></div>' +
    '</div>' +

    '<div style="margin-top:16px;padding:12px 14px;background:var(--bg-2);border-radius:var(--r-sm);border:1px solid var(--border);display:flex;justify-content:center;align-items:center;gap:8px">' +
      '<span style="font-size:11px;color:var(--txt-3)">Built by</span>' +
      '<a href="' + safeBrandUrl + '" target="_blank" rel="noopener noreferrer" style="font-family:var(--font-d);font-size:14px;font-weight:900;letter-spacing:2px;color:var(--lime)">' + TF.Config.appName + '</a>' +
      '<span style="font-size:11px;color:var(--txt-3)">' + TF.Config.version + '</span>' +
    '</div>' +
