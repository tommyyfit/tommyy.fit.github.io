...
TF.UI = (function(){
  ...
  function xpRow(profile){
    ...
    var level=TF.Store.getLevel(p),prog=TF.Store.getXPProgress(p),toNext=TF.Store.getXPToNext(p);
    var shields=TF.Store.getShields();
    return '<div class="xp-row">'+
      '<div class="xp-level-badge">'+level+'</div>'+
      '<div class="xp-details">'+
        '<div class="xp-top"><span class="xp-name">Lv.'+level+' · '+TF.Store.getWarriorTitle(level)+'</span><span class="xp-pts t-mono">'+p.xp+' XP</span></div>'+
        bar(prog,'var(--lime)')+
        '<div class="t-hint mt-1">'+toNext+' XP to level '+(level+1)+(shields>0?' &nbsp;·&nbsp; <span style="color:var(--blue)">🛡 '+shields+' shield'+(shields>1?'s':'')+'</span>':'')+'</div>'+
      '</div>'+
